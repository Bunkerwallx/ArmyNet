...
void aes_encrypt_file(const FileObject& file, const std::array<uint8_t, KEY_SIZE>& key) {
    ...
    try {
        ...
        // Configurar la conexión a Tor
        tor_options_t *options = tor_options_new();
        tor_options_set_socks_port(options, 9050);

        // Crear un circuito Tor
        tor_circuit_t *circuit = tor_circuit_new(options, 0);

        // Redirigir el tráfico a través de Tor
        std::unique_ptr<StreamSocket> socket(new StreamSocket(circuit));
        ...
        // Liberar recursos de Tor
        tor_circuit_free(circuit);
        tor_options_free(options);
        tor_fini();
...
int main(int argc, char **argv)
    ...
    try {
        // Cifra el archivo usando Tor
        aes_encrypt_file(file, key);
        std::cout << "El archivo ha sido cifrado exitosamente usando Tor!" << std::endl;
